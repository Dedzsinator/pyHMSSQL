...
class MainWindow:
    ...
    def create_widgets(self):
        ...
        tk.Button(self.root, text="Create", command=self.create_table).grid(
            row=2, column=6, padx=10, pady=10
        )

        # Drop Table Section
        tk.Label(self.root, text="Drop Table").grid(
        ...
        tk.Button(self.root, text="Drop", command=self.drop_table).grid(
    ...
    def drop_table(self):
        ...
        send_data(
            self.sock,
            {"action": "drop_table", "db_name": db_name, "table_name": table_name},
        )
        response = receive_data(self.sock)
        messagebox.showinfo("Response", response["response"])
