...
class LightweightOrchestrator:
        ...
        for node_id, node in self.cluster_state.items():
            stats[node_id] = {
                ...
                "replication": {
                    ...
                    "status": (
                        "active" if node.health == NodeHealth.HEALTHY else "degraded"
                    ),
                    "master_node": None,
                    "replica_nodes": [],
                },
                ...
                "history": {"cpu": [30.0], "memory": [40.0], "qps": [100.0]},
            }
        return stats
    ...
    def get_topology_graph(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "levels": {
                "primary_masters": [
                    n["id"] for n in nodes if n["role"] == "primary_master"
                ],
    ...
    def get_overview_stats(self) -> Dict[str, Any]:
        ...
        primary_masters = sum(
            1 for n in self.cluster_state.values() if n.role == "primary"
        )
        ...
        return {
            ...
            "replicas": replicas,
            "offline_nodes": sum(
                1 for n in self.cluster_state.values() if n.health != NodeHealth.HEALTHY
            ),
            "total_qps": sum(
                100.0 + (int(n.port) % 500) for n in self.cluster_state.values()
            ),
            "total_connections": sum(
                n.connections for n in self.cluster_state.values()
            ),
            "avg_replication_lag": sum(
                n.replication_lag for n in self.cluster_state.values()
            )
            / max(1, total_nodes),
...
def manual_failover():
    ...
    try:
        ...
        if success:
            ...
            return jsonify(
                {"success": False, "error": f"Failover to {target_node} failed"}
            )
