...
def test_catalog_manager_integration():
    ...
    try:
        ...
        print(
            f"   Original: {[r['table_name'] + '(' + str(r['record_count']) + ')' for r in metadata_records]}"
        )

        # Sort by record count (ascending)
        hyperopt_list_sort(metadata_records, key_func=lambda x: x["record_count"])
        print(
            f"   By count: {[r['table_name'] + '(' + str(r['record_count']) + ')' for r in metadata_records]}"
        )
...
def test_select_executor_integration():
    ...
    try:
        ...
        print(
            f"   Original: {[r['name'] + '(' + str(r['score']) + ')' for r in query_results]}"
        )
        ...
        print(
            f"   By score DESC: {[r['name'] + '(' + str(r['score']) + ')' for r in results_desc]}"
        )
        ...
        print(
            f"   By rank ASC: {[r['name'] + '(' + str(r['rank']) + ')' for r in results_asc]}"
        )
...
def test_performance_with_large_dataset():
    ...
    try:
        ...
        if hyperopt_time < builtin_time:
            ...
            print(
                f"   ⚠️ Overhead: {ratio:.2f}x slower (expected for current implementation)"
            )
