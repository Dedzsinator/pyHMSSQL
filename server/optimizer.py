...
class Optimizer:
    ...
    def optimize_select(self, plan):
        ...
        if condition:
            ...
            if index:
                plan['use_index'] = True
                plan['index'] = index
                logging.debug(f"Using index for column: {column} in table: {table_name}")
    ...
    def optimize_join(self, plan):
        ...
        if plan['type'] == "JOIN":
            ...
            if '=' in condition:  # Equality-based join condition
                ...
                logging.debug(f"Optimized join to HASH_JOIN with condition: {condition}")
    ...
    def merge_projections(self, plan):
        ...
        if plan['type'] == "PROJECT" and 'child' in plan:
            if plan['child']['type'] == "PROJECT":
                plan['columns'] = list(set(plan['columns'] + plan['child']['columns']))
    ...
    def optimize_index_join(self, plan):
        ...
        if plan['type'] == "JOIN":
            ...
            index1 = self.index_manager.get_index(f"{plan['table1']}.idx_{col1}")
            index2 = self.index_manager.get_index(f"{plan['table2']}.idx_{col2}")
            
            if index1 or index2:
                plan['type'] = "INDEX_JOIN"
                plan['index'] = index1 if index1 else index2
                logging.debug(f"Optimized join to INDEX_JOIN with condition: {condition}")
    ...
    def rewrite_expression(self, plan):
        ...
        if plan['type'] == "JOIN":
            ...
            if col1.startswith("#0") and col2.startswith("#0"):
                plan['condition'] = f"#0.{col1.split('.')[1]} = #1.{col2.split('.')[1]}"
    ...
    def optimize_order_by(self, plan):
        ...
        if plan['type'] == "ORDER_BY":
            index = self.index_manager.get_index(f"{plan['table']}.idx_{plan['column']}")
            if index:
                plan['type'] = "INDEX_SCAN"
                plan['index'] = index
                logging.debug(f"Optimized ORDER BY to INDEX_SCAN on column: {plan['column']}")
    ...
    def optimize_sort_limit(self, plan):
        ...
        if plan['type'] == "SORT" and 'limit' in plan:
            ...
            plan['n'] = plan['limit']
            del plan['limit']
            logging.debug(f"Optimized SORT + LIMIT to TOP_N with limit: {plan['n']}")
    ...
    def optimize_predicate_pushdown(self, plan):
        ...
        if plan['type'] == "FILTER" and 'child' in plan:
            if plan['child']['type'] in ["SEQ_SCAN", "INDEX_SCAN"]:
                plan['child']['filter'] = plan['condition']
                logging.debug(f"Pushed down filter condition: {plan['condition']}")
                return plan['child']
    ...
    def optimize_reorder_join(self, plan):
        ...
        if plan['type'] == "JOIN":
            ...
            if index1 and not index2:
                ...
                logging.debug(f"Reordered join tables to use index on: {plan['table1']}")
