...
class Optimizer:
    ...
    def optimize(self, plan):
        ...
        # Log input plan with limited depth to avoid verbose logging
        logging.info(
        ...
        if plan["type"] == "SELECT":
            ...
            optimized_plan = self._apply_generic_optimizations(plan)
        
        # Check if parallel execution would be beneficial
        if self.parallel_coordinator.is_parallelism_beneficial(optimized_plan):
            optimized_plan["parallel_execution"] = True
            optimized_plan["parallel_partitions"] = self.parallel_coordinator.get_optimal_partition_count(
                optimized_plan.get("estimated_rows", 1000)
            )
            logging.info(
                f"Enabling parallel execution with {optimized_plan['parallel_partitions']} partitions"
            )
