...
class CircularPlanCache:
    ...
    def get(self, query_signature: str, tables_used: List[str] = None) -> Optional[Any]:
        ...
        with self.lock:
            ...
            for table in entry.metadata.get("tables_used", []):
                if table in self.table_versions and self.table_versions[
                    table
                ] > entry.metadata.get("schema_version", 0):
                    del self.lookup_table[cache_key]
                    self.invalidations += 1
                    self.misses += 1
                    return None
    ...
    def invalidate(self, query_signature: str = None, table_name: str = None):
        ...
        with self.lock:
            if query_signature:
                ...
                self.table_versions[table_name] = (
                    self.table_versions.get(table_name, 0) + 1
                )
    ...
    def _generate_cache_key(
        self, query_signature: str, tables_used: List[str] = None
    ) -> str:
...
class CacheManager:
    ...
    def __init__(
        self, plan_cache_capacity: int = 1000, result_cache_capacity: int = 500
    ):
        """
    ...
    def get_combined_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_memory_usage": plan_stats["memory_usage_bytes"]
            + result_stats["memory_usage_bytes"],
        }
