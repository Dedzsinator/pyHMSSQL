...
class AccessPathSelector:
    ...
    def select_access_path(self, table_name: str, predicates: List[Predicate],
                          columns_needed: List[str], limit: Optional[int] = None) -> AccessPath:
        """
        ...
        """
        logging.debug(f"Selecting access path for table {table_name} with {len(predicates)} predicates")
        ...
        for index in available_indexes:
            ...
            if applicable_predicates:
                ...
                index_scan_path = self._evaluate_index_scan(
                    table_name, index, applicable_predicates, columns_needed, table_stats, limit
                )
        ...
        bitmap_paths = self._evaluate_bitmap_scans(
            table_name, available_indexes, predicates, columns_needed, table_stats, limit
        )
        ...
        if not valid_paths:
            ...
            logging.warning(f"No valid access paths found for {table_name}, using table scan")
        ...
        elif best_path.method in [AccessMethod.INDEX_SCAN, AccessMethod.INDEX_ONLY_SCAN]:
            self.index_scans_chosen += 1
        elif best_path.method in [AccessMethod.BITMAP_HEAP_SCAN, AccessMethod.BITMAP_INDEX_SCAN]:
            self.bitmap_scans_chosen += 1
        
        logging.debug(f"Selected {best_path.method.value} for {table_name} "
    ...
    def _get_available_indexes(self, table_name: str) -> List[IndexInfo]:
        ...
        try:
            ...
            table_indexes = getattr(self.index_manager, 'get_table_indexes', lambda x: {})(table_name)
    ...
    def _evaluate_table_scan(self, table_name: str, predicates: List[Predicate],
                            columns_needed: List[str], table_stats: Any, 
                            limit: Optional[int]) -> AccessPath:
        """Evaluate a sequential table scan."""
        # Get base table scan cost
        scan_cost = self.cost_estimator.estimate_table_scan_cost(table_name, table_stats)
    ...
    def _evaluate_index_scan(self, table_name: str, index: IndexInfo,
                           predicates: List[Predicate], columns_needed: List[str],
                           table_stats: Any, limit: Optional[int]) -> AccessPath:
        ...
        if remaining_predicates:
            ...
            filter_cost = self.cost_estimator.estimate_filter_cost(
                estimated_rows, table_stats.avg_row_length, 
                [p.column for p in remaining_predicates], remaining_selectivity
            )
    ...
    def _can_use_index_only_scan(self, index: IndexInfo, columns_needed: List[str],
    ...
    def _evaluate_index_only_scan(self, table_name: str, index: IndexInfo,
                                predicates: List[Predicate], columns_needed: List[str],
                                table_stats: Any, limit: Optional[int]) -> AccessPath:
        ...
        cpu_cost = estimated_rows * self.cost_estimator.cost_factors.cpu_index_tuple_cost
    ...
    def _evaluate_bitmap_scans(self, table_name: str, indexes: List[IndexInfo],
                             predicates: List[Predicate], columns_needed: List[str],
                             table_stats: Any, limit: Optional[int]) -> List[AccessPath]:
        ...
        bitmap_predicates = [p for p in predicates 
                           if self.bitmap_scan_threshold <= p.selectivity <= 0.5]
        ...
        for index in indexes:
            applicable_predicates = [p for p in bitmap_predicates if p.can_use_index(index)]
            
            if applicable_predicates:
                bitmap_path = self._create_bitmap_scan_path(
                    table_name, index, applicable_predicates, predicates,
                    columns_needed, table_stats, limit
                )
                if bitmap_path:
    ...
    def _create_bitmap_scan_path(self, table_name: str, index: IndexInfo,
                                bitmap_predicates: List[Predicate], all_predicates: List[Predicate],
                                columns_needed: List[str], table_stats: Any,
                                limit: Optional[int]) -> Optional[AccessPath]:
        ...
        bitmap_creation_cost = estimated_rows * self.cost_estimator.cost_factors.cpu_index_tuple_cost
        
        # 2. Scan heap using bitmap (more efficient than random access)
        heap_pages = max(1, int(estimated_rows * table_stats.avg_row_length / 
                               self.cost_estimator.cost_factors.page_size))
        heap_scan_cost = heap_pages * self.cost_estimator.cost_factors.seq_page_cost * 1.5
        ...
        if remaining_predicates:
            ...
            for predicate in remaining_predicates:
                remaining_selectivity *= predicate.selectivity
            estimated_rows = int(estimated_rows * remaining_selectivity)
            remaining_cost = estimated_rows * self.cost_estimator.cost_factors.cpu_tuple_cost
    ...
    def _evaluate_multi_index_scans(self, table_name: str, indexes: List[IndexInfo],
                                  predicates: List[Predicate], columns_needed: List[str],
                                  table_stats: Any, limit: Optional[int]) -> List[AccessPath]:
        ...
        if len(usable_indexes) >= 2:
            ...
            intersection_path = self._create_index_intersection_path(
                table_name, [(index1, preds1), (index2, preds2)],
                predicates, columns_needed, table_stats, limit
            )
    ...
    def _create_index_intersection_path(self, table_name: str, 
                                      index_predicate_pairs: List[Tuple[IndexInfo, List[Predicate]]],
                                      all_predicates: List[Predicate], columns_needed: List[str],
                                      table_stats: Any, limit: Optional[int]) -> Optional[AccessPath]:
        ...
        for index, predicates in index_predicate_pairs:
            ...
            intersection_cost += index_rows * self.cost_estimator.cost_factors.cpu_index_tuple_cost
        
        # Cost to intersect the bitmaps
        intersection_cost += estimated_rows * self.cost_estimator.cost_factors.cpu_operator_cost * 2
    ...
    def _evaluate_parallel_scans(self, table_name: str, predicates: List[Predicate],
                               columns_needed: List[str], table_stats: Any,
                               limit: Optional[int]) -> List[AccessPath]:
        ...
        # Get base table scan cost
        base_scan = self._evaluate_table_scan(table_name, predicates, columns_needed, table_stats, limit)
    ...
    def explain_access_path(self, path: AccessPath) -> str:
        ...
        if path.predicates_used:
            pred_strs = [f"{p.column} {p.operator} {p.value}" for p in path.predicates_used]
    ...
    def get_selection_statistics(self) -> Dict[str, Any]:
        ...
        total_selections = self.table_scans_chosen + self.index_scans_chosen + self.bitmap_scans_chosen
