...
class MockCatalogManager:
    ...
    def __init__(self):
        ...
        self.tables = {
            ...
            'orders': {'columns': ['id', 'user_id', 'amount', 'date'], 'row_count': 50000},
...
class MockIndexManager:
    ...
    def list_indexes(self, table_name=None):
        if table_name:
            return [idx for name, idx in self.indexes.items() if idx['table'] == table_name]
...
def test_unified_optimizer_integration():
    ...
    try:
        from server.unified_optimizer import UnifiedQueryOptimizer, OptimizationLevel, OptimizationOptions
        ...
        try:
            ...
            for query_test in test_queries:
                ...
                try:
                    ...
                    result = optimizer.optimize(query_plan, query_signature=f"{level.value}_{query_name}")
                    optimization_time = (time.time() - start_time) * 1000
                    
                    print(f"  ✓ {query_name}: {optimization_time:.1f}ms, "
            ...
            if result2.cache_hit:
                print(f"  ✓ Cache working: {result2.optimization_time_ms:.1f}ms vs {result1.optimization_time_ms:.1f}ms")
            else:
                print("  ⚠ Cache not working as expected")
            
            # Get optimizer statistics
            stats = optimizer.get_optimization_statistics()
            print(f"  Stats: {stats.get('total_optimizations', 0)} optimizations, "
    ...
    for level, results in test_results.items():
        if results['success']:
            ...
            avg_time = sum(r['time_ms'] for r in results['results'] if r['success']) / max(1, level_success)
            
            print(f"{level.value:>12}: {level_success}/{level_total} tests passed, "
