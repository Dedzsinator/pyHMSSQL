...
class TriggerManager:
    ...
    def execute_create_trigger(self, plan):
        ...
        if not trigger_name:
            return {"error": "No trigger name specified", "status": "error"}

        if not table:
            return {"error": "No table specified for trigger", "status": "error"}
        ...
        if table not in tables:
            return {"error": f"Table '{table}' does not exist", "status": "error"}

        # Validate trigger body
        if not body.strip():
            return {"error": "Trigger body cannot be empty", "status": "error"}
        ...
        try:
            ...
            return {"error": f"Error creating trigger: {str(e)}", "status": "error"}
    ...
    def execute_drop_trigger(self, plan):
        ...
        try:
            ...
            return {"error": f"Error dropping trigger: {str(e)}", "status": "error"}
    ...
    def fire_triggers(
        ...
        try:
            ...
            for trigger_name, trigger_def in triggers.items():
                if (
                    trigger_def.get("event") == event.upper()
                    and trigger_def.get("timing") == timing.upper()
                ):
            ...
            for statement in statements:
                if statement.strip():
                    try:
                        ...
                        self.logger.error(
                            f"Error executing trigger statement: {str(e)}"
                        )
    ...
    def list_triggers(self, table: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"triggers": triggers, "status": "success"}
        except Exception as e:
            return {"error": f"Error listing triggers: {str(e)}", "status": "error"}
