...
class PlanBaseline:
    ...
    def is_reliable(self) -> bool:
        ...
        return (
            ...
            and self.performance_trend != "degrading"
        )
...
class AdaptiveCostModel:
    ...
    def _adjust_parameters(self):
        ...
        cost_errors = [
            f for f in recent_feedback if f.feedback_type == FeedbackType.COST_ESTIMATE
        ]
        ...
        if avg_error > 0.2:  # 20% threshold
            ...
            logging.info(
                f"Adjusted cost factors by {adjustment:.3f} due to "
...
class RuntimePlanSwitcher:
    ...
    def check_switch_opportunity(
        self, query_id: str, current_progress: float
    ) -> Optional[Dict]:
        """
        ...
        with self._lock:
            ...
            if expected_progress > self.switch_threshold and current_progress < 0.5:
                ...
                if alternatives:
                    ...
                    logging.info(
                        f"Switching query {query_id} to alternative plan - "
...
class AdaptiveOptimizer:
        ...
        with self._lock:
            ...
            if plan_baseline:
                plan_baseline.update_performance(
                    feedback.execution_time, feedback.error_message is None
                )
    ...
    def create_plan_baseline(
        self, query_signature: str, plan: Dict[str, Any]
    ) -> PlanBaseline:
        ...
        with self._lock:
            ...
            existing_baselines = [
                b
                for b in self.plan_baselines.values()
    ...
    def should_use_baseline(
        self, query_signature: str, new_plan: Dict[str, Any]
    ) -> bool:
        ...
        return (
            baseline.execution_count >= 10
            and baseline.success_rate >= 0.95
            and baseline.performance_trend != "degrading"
        )
    ...
    def register_query_execution(
        ...
        if query_signature not in [
            b.query_signature for b in self.plan_baselines.values()
        ]:
    ...
    def _check_performance_alerts(self, feedback: ExecutionFeedback):
        ...
        if (
            feedback.feedback_type == FeedbackType.CARDINALITY_ESTIMATE
            and feedback.is_significant_error
        ):
            alert = {
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "reliable_baselines": len(
                    [b for b in self.plan_baselines.values() if b.is_reliable]
                ),
                "recent_alerts": len(
                    [
                        a
    ...
    def get_recent_alerts(self, hours: int = 24) -> List[Dict]:
        ...
        return [
            alert for alert in self.performance_alerts if alert["timestamp"] >= cutoff
        ]
    ...
    def _cleanup_old_data(self):
        ...
        with self._lock:
            ...
            to_remove = [
                plan_id
                for plan_id, baseline in self.plan_baselines.items()
            ...
            self.performance_alerts = [
                a for a in self.performance_alerts if a["timestamp"] >= alert_cutoff
            ]
    ...
    def _save_state(self):
        ...
        try:
            ...
            recent_feedback = (
                self.feedback_collector[-1000:] if self.feedback_collector else []
            )
    ...
    def _load_state(self):
        ...
        try:
            ...
            logging.info(
                f"Loaded adaptive optimizer state: "
