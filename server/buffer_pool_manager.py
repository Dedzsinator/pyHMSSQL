...
class QueryResultCache:
        ...
        with self.lock:
            if query_hash in self.cache:
                ...
                for table in affected_tables:
                    current_version = self.table_versions.get(table, 0)
                    cached_version = versions.get(table, 0)
                    if current_version > cached_version:
                        # Table has been modified since last cache
                        self.logger.info(f"Cache miss due to version change for table {table} ({cached_version} -> {current_version})")
                        self._remove_entry(query_hash)
                        self.stats["misses"] += 1
                        return None
    ...
    def update_table_version(self, table_name):
        ...
        with self.version_lock:
            current = self.table_versions.get(table_name, 0)
            self.table_versions[table_name] = current + 1
            self.logger.info(f"Updated version for table {table_name}: {current} -> {current+1}")
            return current + 1
