...
class BPTreeVisualizer:
    ...
    def _visualize_networkx(self, tree, output_name):
        ...
        nx.draw(G, pos, node_color=node_colors, node_size=2500,
    ...
    def _visualize_matplotlib(self, tree, output_name):
        ...
        self._draw_node(ax, tree.root, level=0, x_pos=50, y_pos=90, 
                      width=80, level_height=15)
    ...
    def _draw_node(self, ax, node, level, x_pos, y_pos, width, level_height):
        ...
        if is_leaf:
            ...
            node_text = ", ".join([f"{k}:{v}" if isinstance(k, tuple) else str(k) 
                                 for k in node.keys])
        else:
        ...
        # Draw the node
        rect = patches.Rectangle((x_pos - box_width/2, y_pos - 3), 
                                box_width, 6, linewidth=1, 
        ...
        if not is_leaf and hasattr(node, 'children') and node.children:
            ...
            for i, child in enumerate(children):
                ...
                # Draw line to child
                ax.plot([x_pos, child_x], [y_pos - 3, child_y + 3], 
                      color='black', linestyle='-', linewidth=1)
                
                # Recursively draw the child
                self._draw_node(ax, child, level + 1, child_x, child_y, 
                             child_width, level_height)
