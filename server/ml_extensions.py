...
class QueryFeatures:
    ...
    def to_vector(self) -> np.ndarray:
        ...
        index_ratio = (
            np.mean(self.index_availability) if self.index_availability else 0.0
        )
...
class CardinalityModel:
    ...
    def extract_features(self, query_plan: Dict, statistics_collector) -> QueryFeatures:
        ...
        def extract_from_plan(plan: Dict):
            ...
            if plan_type in ("SELECT", "TABLE_SCAN", "INDEX_SCAN"):
                ...
                if table_name:
                    try:
                        stats = statistics_collector.collect_table_statistics(
                            table_name
                        )
    ...
    def add_training_example(
        self, query_features: QueryFeatures, actual_cardinality: int
    ):
    ...
    def train_model(self):
        ...
        try:
            ...
            logging.info(
                f"Trained cardinality model on {len(self.training_data)} examples"
            )
    ...
    def load_model(self):
        ...
        try:
            with open(self.model_path, "rb") as f:
                ...
                logging.info(
                    f"Loaded cardinality model (trained on {data.get('training_data_count', 0)} examples)"
                )
        except Exception as e:
...
class JoinOrderPolicyModel:
    ...
    def predict_join_order(
        self, tables: List[str], join_graph: Dict, statistics_collector
    ) -> List[str]:
    ...
    def _extract_table_features(
        self, table: str, all_tables: List[str], join_graph: Dict, statistics_collector
    ) -> np.ndarray:
...
class MLExtensionManager:
    ...
    def __init__(
        self,
        enable_cardinality_model: bool = True,
        ...
        logging.info(
            f"ML Extensions initialized - Cardinality: {self.enable_cardinality_model}, "
            f"Join Order: {self.enable_join_order_model}"
        )

    def estimate_cardinality(
        self, query_plan: Dict, statistics_collector
    ) -> Optional[int]:
        ...
        try:
            features = self.cardinality_model.extract_features(
                query_plan, statistics_collector
            )
    ...
    def add_execution_feedback(
        ...
        if self.cardinality_model:
            try:
                ...
                self.cardinality_model.add_training_example(
                    features, actual_cardinality
                )
            except Exception as e:
    ...
    def get_model_status(self) -> Dict[str, Any]:
        ...
        if self.cardinality_model:
            ...
            status["cardinality_training_examples"] = len(
                self.cardinality_model.training_data
            )
