...
class SelectExecutor:
    ...
    def execute_select(self, plan):
        ...
        # Initial logging
        logging.debug(
            "Executing SELECT with plan: %s",
            {k: v for k, v in plan.items() if k != "query"},
        )
        ...
        if condition and isinstance(condition, str):
            ...
            if "=" in condition and not any(
                op in condition for op in ["<", ">", "AND", "OR"]
            ):
                ...
                if len(parts) == 2:
                    ...
                    logging.info(
                        "Checking for index on column: %s with value: %s", col, val
                    )
                    index_column = col
                    index_value = val
            ...
            # Check for range conditions (e.g., "age >= 25 AND age <= 30")
                ...
                range_match = re.search(
                    r"(\w+)\s*[><=]+\s*\d+\s*AND\s*\1\s*[><=]+\s*\d+", condition
                )
                if range_match:
        ...
        try:
            ...
            if condition and index_column:
                ...
                logging.info(
                    "DEBUG: condition=%s, index_column=%s, indexes=%s",
                    condition,
                    index_column,
                    indexes,
                )
                for idx_name, idx_def in indexes.items():
                    if idx_def.get("column").lower() == index_column.lower():
                        ...
                        logging.info(
                            "Will use index %s for condition %s with scan type: %s",
                            idx_name,
                            index_column,
                            plan.get("scan_type"),
                        )
                    ...
                    logging.info(
                        "DEBUG: Setting scan_type to FULL_SCAN (no index found)"
                    )
                    plan["scan_type"] = "FULL_SCAN"
            else:
                # No condition or no indexable column, this will be a full table scan
                logging.info(
                    "DEBUG: Setting scan_type to FULL_SCAN (no condition or index_column)"
                )
                plan["scan_type"] = "FULL_SCAN"
            ...
            logging.debug(
                f"Should check for cached B+ tree index for table: {actual_table_name}"
            )
            ...
            if "parsed_condition" in plan and hasattr(self, "execution_engine"):
                ...
                if all_records:
                    for record in all_records:
                        if self.execution_engine._evaluate_condition(
                            record, plan["parsed_condition"]
                        ):
                            results.append(record)

                    logging.info(
                        f"Found {len(results)} matching records after filtering"
                    )
            else:
                    ...
                    for cond in conditions:
                        ...
                        if isinstance(val, str):
                            if (val.startswith("'") and val.endswith("'")) or (
                                ...
                                logging.info(
                                    "Removed quotes from condition value: %s",
                                    cond["value"],
                                )
            ...
            if order_by and results:
                ...
                if order_column:
                    ...
                    logging.info(
                        f"Sorting by {order_column} {direction}, reverse={reverse}"
                    )
                    ...
                    try:
                        ...
                        hyperopt_list_sort(
                            results, key_func=get_sort_key, reverse=reverse
                        )
                    except Exception:
            ...
            if "*" in columns:
                ...
                if results and isinstance(results[0], dict) and results[0]:
                    ...
                    logging.info(
                        "SELECT * will return these columns: %s", result_columns
                    )
            ...
            logging.info(
                "Final result: %s rows with columns: %s",
                len(result_rows),
                result_columns,
            )
