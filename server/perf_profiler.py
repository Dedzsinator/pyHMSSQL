...
class PerfProfiler:
    ...
    def __init__(self, output_dir: str = None):
        ...
        logging.info(
            f"PerfProfiler initialized with output directory: {self.output_dir}"
        )

    def start_cpu_profiling(
        self, session_name: str, frequency: int = 99, call_graph: str = "dwarf"
    ) -> bool:
        """
        ...
        try:
            ...
            logging.info(
                f"Started CPU profiling for session '{session_name}' (PID: {process.pid})"
            )
            return True
    ...
    def start_counter_profiling(
        self, session_name: str, counters: List[str] = None, interval: float = 1.0
    ) -> bool:
        """
        ...
        if session_name in self._active_processes:
            logging.warning(
                f"Counter profiling session '{session_name}' already active"
            )
            return False
        ...
        cmd = [
            ...
            ",".join(counters),
            "-I",
            str(int(interval * 1000)),  # Convert to milliseconds
            "-x",
            ",",  # CSV output
            "--pid",
            str(os.getpid()),
        ...
        try:
            ...
            logging.info(
                f"Started counter profiling for session '{session_name}' (PID: {process.pid})"
            )
            return True
    ...
    def start_memory_profiling(self, session_name: str) -> bool:
        ...
        try:
            ...
            logging.info(
                f"Started memory profiling for session '{session_name}' (PID: {process.pid})"
            )
            return True

        except Exception as e:
            logging.warning(
                f"Memory profiling may not be available on this system: {e}"
            )
            return False

    def stop_profiling(
        self, session_name: str, profile_type: str = "all"
    ) -> Dict[str, Any]:
        """
        ...
        for ptype in types_to_stop:
            ...
            if key in self._active_processes:
                ...
                try:
                    ...
                    logging.info(
                        f"Stopped {ptype} profiling for session '{session_name}'"
                    )
    ...
    def _process_counter_results(self, session_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for counter, values in counters.items():
                numeric_values = [
                    v["value"] for v in values if isinstance(v["value"], int)
                ]
                if numeric_values:
                    stats[counter] = {
    ...
    def _get_available_events(self) -> List[str]:
        ...
        try:
            result = subprocess.run(
                ["perf", "list"], capture_output=True, text=True, timeout=10
            )
    ...
    def _check_perf_capabilities(self) -> Dict[str, bool]:
        ...
        try:
            result = subprocess.run(
                ["perf", "record", "--help"], capture_output=True, timeout=5
            )
        ...
        try:
            result = subprocess.run(
                ["perf", "stat", "--help"], capture_output=True, timeout=5
            )
        ...
        try:
            result = subprocess.run(
                ["perf", "stat", "-e", "cycles", "true"], capture_output=True, timeout=5
            )
...
class EnhancedSystemProfiler:
    ...
    def stop_comprehensive_profiling(self, session_name: str) -> Dict[str, Any]:
        ...
        if (
            self.perf_enabled
            and self.perf_profiler
            and session_name in self._active_perf_sessions
        ):
