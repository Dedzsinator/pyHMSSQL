...
class JoinPlan:
    ...
    def is_base_table(self) -> bool:
        ...
        return (
            len(self.tables) == 1 and self.left_plan is None and self.right_plan is None
        )
    ...
    def get_plan_signature(self) -> str:
        ...
        if self.join_type == JoinType.INNER and self.join_algorithm in [
...
class MemoizationTable:
    ...
    def store_plan(self, plan: JoinPlan):
        ...
        if not self._is_dominated(plan, self.plans[table_set]):
            ...
            self.plans[table_set] = [
                p for p in self.plans[table_set] if not self._dominates(plan, p)
            ]
            ...
            if (
                table_set not in self.best_plans
                or plan.estimated_cost < self.best_plans[table_set].estimated_cost
            ):
...
class JoinOrderEnumerator:
    ...
    def enumerate_join_orders(
        ...
        try:
            ...
            logging.info(
                f"Enumerating join orders for {len(tables)} tables using {strategy} strategy"
            )
            ...
            logging.info(
                f"Join enumeration completed in {self.enumeration_time:.3f}s, "
    ...
    def _build_join_graph(
        self, tables: List[str], join_conditions: List[JoinCondition]
    ) -> Dict[str, Set[str]]:
        ...
        for condition in join_conditions:
            if (
                condition.left_table in join_graph
                and condition.right_table in join_graph
            ):
    ...
    def _enumerate_left_deep(
        self, tables: List[str], join_graph: Dict[str, Set[str]]
    ) -> JoinPlan:
        ...
        for start_table in tables:
            ...
            for ordering in permutations(remaining_tables):
                plan = self._build_left_deep_plan(
                    start_table, list(ordering), join_graph
                )
        ...
        while remaining_tables:
            ...
            for table in remaining_tables:
                if self._can_join(current_plan.tables, {table}, join_graph):
                    candidate_plan = self._create_join_plan(
                        current_plan, self._create_base_plan(table), join_graph
                    )
            ...
            if best_next_table:
                ...
                current_plan = self._create_cross_join_plan(
                    current_plan, self._create_base_plan(next_table)
                )
                self.plans_generated += 1

        return current_plan

    def _generate_plans_for_subset(
        self, subset: FrozenSet[str], join_graph: Dict[str, Set[str]]
    ):
                ...
                for left_plan in left_plans[:3]:  # Limit to top 3 plans for efficiency
                    for right_plan in right_plans[:3]:
                        join_plan = self._create_join_plan(
                            left_plan, right_plan, join_graph
                        )
    ...
    def _find_join_condition(
        ...
        for condition in join_conditions:
            if (
                condition.left_table in left_tables
                and condition.right_table in right_tables
            ):
                return condition
            elif (
                condition.right_table in left_tables
                and condition.left_table in right_tables
            ):
                # Flip the condition
    ...
    def _create_join_plan(
        self, left_plan: JoinPlan, right_plan: JoinPlan, join_graph: Dict[str, Set[str]]
    ) -> Optional[JoinPlan]:
    ...
    def _create_cross_join_plan(
        self, left_plan: JoinPlan, right_plan: JoinPlan
    ) -> JoinPlan:
        """Create a cross join plan (Cartesian product)."""
        estimated_rows = left_plan.estimated_rows * right_plan.estimated_rows
        estimated_cost = (
            left_plan.estimated_cost + right_plan.estimated_cost + estimated_rows
        )
    ...
    def _choose_join_algorithm(
        self, left_plan: JoinPlan, right_plan: JoinPlan, join_condition: JoinCondition
    ) -> JoinAlgorithm:
    ...
    def _build_left_deep_plan(
        ...
        for table in remaining_tables:
            ...
            if self._can_join(current_plan.tables, frozenset([table]), join_graph):
                current_plan = self._create_join_plan(
                    current_plan, right_plan, join_graph
                )
                if not current_plan:
                    return None
            else:
    ...
    def _create_fallback_plan(
        ...
        for i in range(1, len(tables)):
            ...
            for condition in join_conditions:
                if (
                    condition.left_table in current_plan.tables
                    and condition.right_table == tables[i]
                ) or (
                    condition.right_table in current_plan.tables
                    and condition.left_table == tables[i]
                ):
            ...
            if join_condition:
                ...
                estimated_cost = (
                    current_plan.estimated_cost + right_plan.estimated_cost + 1000
                )
                estimated_rows = int(
                    current_plan.estimated_rows * right_plan.estimated_rows * 0.1
                )
            else:
                # Cross join
                join_algorithm = JoinAlgorithm.NESTED_LOOP
                estimated_cost = (
                    current_plan.estimated_cost
                    + right_plan.estimated_cost
    ...
    def explain_plan(self, plan: JoinPlan, indent: int = 0) -> str:
        ...
        result += (
            f"{spaces}  Cost: {plan.estimated_cost:.1f}, Rows: {plan.estimated_rows}\n"
        )

        if plan.join_condition:
            result += (
                f"{spaces}  Condition: {plan.join_condition.left_table}.{plan.join_condition.left_column} "
