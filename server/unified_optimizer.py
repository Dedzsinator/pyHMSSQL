...
try:
    from server.adaptive_optimizer import AdaptiveOptimizer, ExecutionFeedback, FeedbackType
    ADAPTIVE_AVAILABLE = True
except ImportError:
    try:
        from adaptive_optimizer import AdaptiveOptimizer, ExecutionFeedback, FeedbackType
        ADAPTIVE_AVAILABLE = True
    except ImportError:
        ADAPTIVE_AVAILABLE = False
        logging.warning("Adaptive optimizer not available - runtime adaptivity disabled")
...
class UnifiedQueryOptimizer:
    ...
    def __init__(self, catalog_manager, index_manager, 
        ...
        self.statistics_collector = AdvancedStatisticsCollector(catalog_manager, index_manager)
        ...
        if self.options.enable_caching:
            ...
            plan_capacity = max(100, cache_size // (2 * 1024))  # Rough estimate for plan cache
            result_capacity = max(50, cache_size // (4 * 1024))  # Rough estimate for result cache
            self.cache_system = CircularCacheSystem(
                plan_cache_capacity=plan_capacity,
        ...
        logging.info(f"Unified Query Optimizer initialized with level: {self.options.level}")
        logging.info(f"Components enabled - Transformations: {self.transformation_engine is not None}, "
    ...
    def optimize(self, query_plan: Dict[str, Any], 
                 query_signature: str = None) -> OptimizationResult:
        """
        ...
        try:
            ...
            if (not cache_hit and self.adaptive_optimizer and query_signature):
                ...
                if baseline and self.adaptive_optimizer.should_use_baseline(
                    ...
                    logging.debug(f"Using adaptive baseline for query: {query_signature}")
            
            # Step 3: Apply query transformations
            if (not cache_hit and self.transformation_engine and 
                self.options.enable_transformations):
                
                try:
                    # Skip transformations for now - interface needs adjustment
                    logging.debug("Query transformations temporarily disabled pending interface update")
            ...
            if (not cache_hit and self.join_enumerator and 
                self.options.enable_join_reordering):
                ...
                if join_plan and len(join_plan.get('tables', [])) > 1:
                    ...
                    if optimal_join:
                        ...
                        if original_order != new_order:
                            ...
                            optimized_plan = self._apply_join_plan(optimized_plan, optimal_join)
                            self.optimization_stats['join_reorderings'] += 1
                            logging.debug(f"Changed join order from {original_order} to {new_order}")
            
            # Step 5: Access path selection
            if (not cache_hit and self.access_path_selector and 
                self.options.enable_access_path_selection):
                
                access_plans = self.access_path_selector.select_access_paths(optimized_plan)
                if access_plans:
                    optimized_plan = self._apply_access_paths(optimized_plan, access_plans)
                    access_paths_selected = [plan.access_method.value for plan in access_plans]
                    logging.debug(f"Selected access paths: {access_paths_selected}")
            
            # Step 6: ML-based enhancements
            if (not cache_hit and self.ml_extensions and 
                self.options.enable_ml_extensions):
                ...
                if not join_order_changed:
                    ...
                    if len(join_tables) > 2:
                        ...
                        if ml_order and ml_order != join_tables:
                            optimized_plan = self._apply_ml_join_order(optimized_plan, ml_order)
                            join_order_changed = True
                            ml_predictions_used = True
            ...
            if self.adaptive_optimizer and query_signature:
                ...
                self.adaptive_optimizer.register_query_execution(
                    query_id, query_signature, optimized_plan, 
        ...
        if (not cache_hit and self.cache_system and query_signature and 
            ...
            try:
                ...
                logging.debug(f"Cached optimized plan for query signature: {query_signature}")
            except Exception as e:
        ...
        logging.info(f"Query optimization completed in {optimization_time:.1f}ms - "
    ...
    def add_execution_feedback(self, query_signature: str, query_id: str,
                             actual_rows: int, actual_time: float,
        ...
        if query_signature in [b.query_signature for b in self.adaptive_optimizer.plan_baselines.values()]:
            baseline = self.adaptive_optimizer.get_plan_baseline(query_signature)
            if baseline:
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        if self.adaptive_optimizer:
            base_stats['adaptive_status'] = self.adaptive_optimizer.get_performance_summary()
    ...
    def _apply_access_paths(self, plan: Dict[str, Any], access_plans: List) -> Dict[str, Any]:
    ...
    def _apply_ml_join_order(self, plan: Dict[str, Any], ml_order: List[str]) -> Dict[str, Any]:
...
def create_optimizer(catalog_manager, index_manager, 
                    level: OptimizationLevel = OptimizationLevel.STANDARD) -> UnifiedQueryOptimizer:
    """
