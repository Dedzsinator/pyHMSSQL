...
class RaftNode:
    ...
    def _reset_election_timeout(self) -> float:
        ...
        self.logger.debug(
            f"Reset election timeout: timeout={timeout:.2f}s, new_deadline={new_deadline:.2f}, current={time.time():.2f}"
        )
        return new_deadline
    ...
    def _start_server(self):
        ...
        try:
            ...
            self.logger.info(
                f"RAFT node {self.node_id} listening on {self.host}:{self.port}"
            )
                ...
                for entry_data in data.get("entries", []):
                    try:
                        ...
                        if isinstance(entry_data, dict):
                            ...
                            self.logger.warning(
                                f"Invalid entry data format: {entry_data}"
                            )
        ...
        with self.lock:
            ...
            if request.prev_log_index >= 0:
                if (
                    ...
                    or self.log[request.prev_log_index].term != request.prev_log_term
                ):
                    # Log inconsistency - find conflict index
                    conflict_index = min(request.prev_log_index, len(self.log) - 1)
                    while (
                        conflict_index > 0
                        and self.log[conflict_index].term == request.prev_log_term
                    ):
                        conflict_index -= 1
            ...
            if request.entries:
                ...
                for entry in request.entries:
                    ...
                    if actual_checksum != expected_checksum:
                        self.logger.error(
                            f"Checksum mismatch for log entry {entry.index}"
                        )
                        return {
                            "type": MessageType.APPEND_RESPONSE.value,
                            "data": AppendResponse(
    ...
    def _become_follower(self):
        ...
        if self.state != NodeState.FOLLOWER:
            self.logger.info(
                f"Node {self.node_id} becoming follower in term {self.current_term}"
            )
            self.state = NodeState.FOLLOWER
    ...
    def _become_candidate(self):
        ...
        with self.lock:
            ...
            if self.state == NodeState.LEADER:
                self.logger.debug(
                    f"Already leader for term {self.current_term}, not becoming candidate"
                )
                return
            ...
            self.logger.info(
                f"Node {self.node_id} becoming candidate for term {self.current_term} (was {prev_state.value})"
            )
    ...
    def _become_leader(self):
        ...
        with self.lock:
            ...
            self.logger.info(
                f"Node {self.node_id} becoming leader for term {self.current_term}"
            )
    ...
    def _conduct_election(self):
        ...
        with self.lock:
            ...
            self.logger.info(
                f"Election: total_votes={total_votes}, votes_needed={votes_needed}, votes_received={self.votes_received}"
            )
        ...
        while (
            time.time() - start_time < self.election_timeout_min
            and self.state == NodeState.CANDIDATE
            and self.votes_received < votes_needed
            and self.current_term == self.current_election_term
        ):
            time.sleep(0.1)
        ...
        with self.lock:
            self.logger.info(
                f"Election result: state={self.state.value}, votes_received={self.votes_received}, votes_needed={votes_needed}, term_match={self.current_term == self.current_election_term}"
            )
            if (
                self.state == NodeState.CANDIDATE
                and self.votes_received >= votes_needed
    ...
    def _send_vote_request(
        self, node_id: str, node_info: Dict[str, Any], vote_request: VoteRequest
    ):
        ...
        try:
            self.logger.debug(
                f"Sending vote request to {node_id} at {node_info['host']}:{node_info['port']} for term {vote_request.term}"
            )
            ...
            try:
                ...
                self.logger.debug(
                    f"Connection failed to {node_id} at {node_info['host']}:{node_info['port']}: {e}"
                )
                return
            ...
            try:
                ...
                if response:
                    self.logger.debug(
                        f"Received vote response data from {node_id}: {response}"
                    )
                    if response.get("type") == MessageType.VOTE_RESPONSE.value:
                        vote_response = VoteResponse(**response["data"])

                        self.logger.debug(
                            f"Processed vote from {node_id}: granted={vote_response.vote_granted}, term={vote_response.term}"
                        )
                        ...
                        with self.lock:
                            ...
                            if vote_response.term > self.current_term:
                                self.logger.info(
                                    f"Received higher term {vote_response.term} > {self.current_term} from {node_id}, stepping down"
                                )
                                ...
                                and vote_response.vote_granted
                                and self.state == NodeState.CANDIDATE
                                and self.current_term == self.current_election_term
                            ):
                                # Count the vote
                                ...
                                self.logger.info(
                                    f"Received vote from {node_id}, total votes: {self.votes_received}"
                                )
                else:
            ...
            try:
                ...
                if self.state == NodeState.LEADER:
                    ...
                    if time_until_deadline < 2.0:
                        self.logger.debug(
                            f"Election deadline approaching: state={self.state.value}, "
                            f"time_left={time_until_deadline:.2f}s"
                        )

                    if current_time > self.election_deadline:
                        self.logger.info(
                            f"Election timeout triggered: state={self.state.value}, "
                        ...
                        with self.lock:
                            ...
                            if (
                                self.state in [NodeState.FOLLOWER, NodeState.CANDIDATE]
            ...
            if self.log[i].term == self.current_term:
                ...
                for node_id in voting_nodes:
                    if (
                        ...
                        and self.match_index.get(node_id, 0) >= i
                    ):
                        replicated_count += 1
                            ...
                            if i < len(self.log):
                                ...
                                if self.apply_callback:
                                    try:
                                        ...
                                        self.logger.error(
                                            f"Error applying log entry {i}: {e}"
                                        )
                        ...
                        if node_id != self.node_id:
                            ...
                            if current_time - last_seen > 30:  # 30 second timeout
                                if node_info["status"] != "offline":
                                    ...
                                    self.logger.warning(
                                        f"Node {node_id} marked as offline"
                                    )
    ...
    def add_log_entry(self, operation: Dict[str, Any]) -> bool:
        ...
        with self.lock:
            ...
            entry = LogEntry(
                term=self.current_term, index=len(self.log), operation=operation
            )
    ...
    def shutdown(self):
        ...
        if self.server_thread and self.server_thread.is_alive():
            ...
            if self.server_thread.is_alive():
                self.logger.warning(
                    f"Server thread for {self.node_id} did not stop gracefully"
                )
...
class RaftCluster:
    ...
    def add_node(
        self, node_id: str, host: str, port: int, is_local: bool = False
    ) -> RaftNode:
        """Add a node to the cluster"""
        # Get all other nodes for peer list
        peers = [
            (nid, info["host"], info["port"])
        ...
        node.set_leadership_callback(
            lambda is_leader: self._on_leadership_change(node_id, is_leader)
        )
    ...
    def get_cluster_status(self) -> Dict[str, Any]:
        ...
        status = {"nodes": {}, "leader": None, "term": 0}
