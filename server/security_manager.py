...
class SecurityManager:
    ...
    def __init__(self, security_level: SecurityLevel = SecurityLevel.PRODUCTION,
                 cert_dir: str = "/certs", audit_log_dir: str = "/logs/audit"):
        ...
        self.token_expiry_hours = 24 if security_level != SecurityLevel.HIGH_SECURITY else 8

        # User and permission management
        self.users: Dict[str, Dict[str, Any]] = {}
        self.role_permissions: Dict[UserRole, List[Permission]] = self._init_role_permissions()
        ...
        self.logger.info(f"SecurityManager initialized with {security_level.value} security level")
    ...
    def _init_role_permissions(self) -> Dict[UserRole, List[Permission]]:
        ...
        return {
            ...
            UserRole.DBA: [
                ...
                Permission.DROP, Permission.ALTER, Permission.INDEX,
                Permission.BACKUP, Permission.RESTORE, Permission.REPLICATION
            ],
            UserRole.DEVELOPER: [
                Permission.READ, Permission.WRITE, Permission.CREATE,
                Permission.ALTER, Permission.INDEX
            ],
            UserRole.ANALYST: [
                Permission.READ, Permission.INDEX
            ],
            UserRole.READ_ONLY: [Permission.READ],
            UserRole.SERVICE_ACCOUNT: [
                Permission.READ, Permission.WRITE, Permission.REPLICATION
            ]
        }
    ...
    def _ensure_certificates(self) -> Tuple[str, str]:
        ...
        if not cert_file.exists():
            self._generate_server_certificate(cert_file, key_file, ca_cert_file, ca_key_file)
    ...
    def _generate_ca_certificate(self, ca_cert_file: Path, ca_key_file: Path):
        ...
        private_key = rsa.generate_private_key(
            ...
            key_size=4096 if self.security_level == SecurityLevel.HIGH_SECURITY else 2048
        )
        ...
        subject = issuer = x509.Name([
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"pyHMSSQL"),
            x509.NameAttribute(NameOID.COMMON_NAME, u"pyHMSSQL CA"),
        ])
        ...
        cert = x509.CertificateBuilder().subject_name(
            ...
            x509.SubjectAlternativeName([
                ...
                x509.DNSName(u"*.hmssql-cluster.svc.cluster.local"),
            ]),
            critical=False,
        ).add_extension(
    ...
    def _generate_server_certificate(self, cert_file: Path, key_file: Path,
                                    ca_cert_file: Path, ca_key_file: Path):
        ...
        subject = x509.Name([
            ...
            x509.NameAttribute(NameOID.LOCALITY_NAME, u"San Francisco"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, u"pyHMSSQL"),
            x509.NameAttribute(NameOID.COMMON_NAME, u"hmssql-server"),
        ])
        ...
        cert = x509.CertificateBuilder().subject_name(
            ...
            x509.SubjectAlternativeName([
                x509.DNSName(u"localhost"),
                x509.DNSName(u"hmssql-server"),
                x509.DNSName(u"*.hmssql-cluster.svc.cluster.local"),
                x509.DNSName(u"hmssql-cluster-0.hmssql-headless.hmssql-cluster.svc.cluster.local"),
                x509.DNSName(u"hmssql-cluster-1.hmssql-headless.hmssql-cluster.svc.cluster.local"),
                x509.DNSName(u"hmssql-cluster-2.hmssql-headless.hmssql-cluster.svc.cluster.local"),
    ...
    def create_user(self, username: str, password: str, role: UserRole,
                   email: str = None, metadata: Dict[str, Any] = None) -> bool:
    ...
    def authenticate_user(self, username: str, password: str,
                         client_ip: str = None) -> Optional[str]:
        """Authenticate user and return JWT token"""
        try:
            # Check IP restrictions
            if client_ip and not self._check_ip_allowed(client_ip):
                self.logger.warning(f"Authentication attempt from blocked IP: {client_ip}")
                return None
    ...
    def check_permission(self, username: str, permission: Permission,
        ...
        try:
            ...
            if permission in role_permissions:
                self.logger.info(f"Permission granted: {username} {permission.value} on {resource}")
                return True

            self.logger.warning(f"Permission denied: {username} {permission.value} on {resource}")
            return False
    ...
    def audit_log(self, action: str, username: str = None, resource: str = None,
    ...
    def cleanup_expired_sessions(self):
        ...
        for session_id, session_data in self.active_sessions.items():
            ...
            if (current_time - created_at).total_seconds() > (self.token_expiry_hours * 3600):
