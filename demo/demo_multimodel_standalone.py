...
def demo_object_relational():
    ...
    print(f"✅ Incomplete address validation: {validator.validate_value(incomplete_address, address_type)}")
...
def demo_document_store():
    ...
    from server.multimodel.document_store.doc_adapter import JSONPathParser, DocumentCollection
...
def demo_graph_database():
    ...
    print(f"\n✅ Created edges:")
    print(f"   {alice.properties['name']} KNOWS {bob.properties['name']} (since {friendship.properties['since']})")
    print(f"   {alice.properties['name']} WORKS_FOR {company.properties['name']} (as {employment.properties['position']})")
...
def demo_unified_architecture():
    ...
    for record_type, data in record_types:
        ...
        if record_type == RecordType.GRAPH_VERTEX:
            match = (data["id"] == decoded["id"] and 
                    data["label"] == decoded["label"] and
                    data["properties"] == decoded["properties"])
        elif record_type == RecordType.GRAPH_EDGE:
            match = (data["id"] == decoded["id"] and 
                    data["from_vertex"] == decoded["from_vertex"])
        else:
            match = all(k in decoded and decoded[k] == v for k, v in data.items())
        
        print(f"   {record_type.name}: {len(encoded)} bytes - {'✅' if match else '❌'}")
