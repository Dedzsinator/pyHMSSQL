...
class StatsCollector:
        ...
        with self.stats_lock:
            ...
            for semi_master_id in self.semi_masters.keys():
                try:
                    ...
                    self.logger.debug(f"Failed to collect stats from semi-master {semi_master_id}: {e}")
    ...
    def _collect_replication_metrics(self, stats: ServerStats, node_info):
        ...
        try:
            ...
            stats.replication_status = "active" if node_info.health.value == "healthy" else "degraded"
        ...
        for node_id, stats in self.server_stats.items():
            ...
            for replica_id in stats.replica_nodes:
                graph["edges"].append({
                    ...
                    "to": replica_id,
                    "type": "replication",
                    "lag": self.server_stats.get(replica_id, {}).replication_lag or 0
                })
            ...
            for semi_master_id in stats.semi_master_nodes:
                graph["edges"].append({
                    ...
                    "to": semi_master_id,
                    "type": "semi_master",
                    "lag": self.server_stats.get(semi_master_id, {}).replication_lag or 0
                })
    ...
    def _find_replicas_for_master(self, master_id: str) -> List[str]:
        ...
        replicas = []
        for node_id, stats in self.server_stats.items():
            if (stats.role == ServerRole.REPLICA and 
                stats.master_node == master_id):
    ...
    def get_cluster_overview(self) -> Dict[str, Any]:
        ...
        with self.stats_lock:
            ...
            healthy_nodes = sum(1 for s in self.server_stats.values() 
                              if s.health_status == "healthy")
            health_ratio = healthy_nodes / len(self.server_stats) if self.server_stats else 0
