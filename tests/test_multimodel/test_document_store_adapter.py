...
class TestJSONPathParser:
    ...
    def test_extract_paths_simple_document(self):
        ...
        expected_paths = [("name", "John"), ("age", 30), ("active", True)]
    ...
    def test_get_value_by_path(self):
        ...
        assert (
            JSONPathParser.get_value_by_path(doc, "user.profile.contacts.email")
            == "bob@example.com"
        )
    ...
    def test_evaluate_filter_simple_conditions(self):
        ...
        doc = {"name": "Alice", "age": 30, "active": True, "score": 85.5}
    ...
    def test_evaluate_filter_nested_conditions(self):
        ...
        doc = {"user": {"profile": {"name": "Charlie", "age": 28}}}

        assert (
            JSONPathParser.evaluate_filter(doc, {"user.profile.name": "Charlie"})
            == True
        )
        assert (
            JSONPathParser.evaluate_filter(doc, {"user.profile.name": "Dave"}) == False
        )
    ...
    def test_evaluate_filter_comparison_operators(self):
        ...
        doc = {"score": 85, "rating": 4.5, "count": 10}
...
class TestDocumentStoreAdapter:
    ...
    def test_insert_document(self, adapter):
        ...
        doc = {"name": "John Doe", "email": "john@example.com", "age": 30}
    ...
    def test_update_nonexistent_document(self, adapter):
        ...
        updates = {"age": 31}
        result = adapter.update_document(
            "users", "nonexistent_id", updates, "test_schema"
        )
        assert result == False
        ...
        docs = [
            {
                "name": "Laptop",
                "specs": {
                    "cpu": {"brand": "Intel", "model": "i7", "cores": 8},
            ...
            },
            {
                "name": "Desktop",
                "specs": {
                    "cpu": {"brand": "AMD", "model": "Ryzen", "cores": 12},
    ...
    def test_concurrent_operations(self, adapter):
        ...
        def worker_insert(worker_id, num_docs):
            ...
            for i in range(num_docs):
                doc = {"worker": worker_id, "index": i, "data": f"data_{worker_id}_{i}"}
                try:
                    doc_id = adapter.insert_document(
                        "concurrent_test", doc, "perf_schema"
                    )
                    doc_ids.append(doc_id)
        ...
        for worker_id in range(num_workers):
            thread = threading.Thread(
                target=lambda wid=worker_id: results.update(
                    {wid: worker_insert(wid, docs_per_worker)}
                )
            )
        ...
        print(
            f"Concurrent insertion: {total_docs}/{expected_total} docs in {end_time - start_time:.2f}s"
        )
...
class TestDocumentStoreEdgeCases:
    ...
    def test_large_document_handling(self, adapter):
        ...
        large_doc = {
            ...
            "nested": {
                "level1": {
                    "level2": {
                        "level3": {"data": ["item" + str(i) for i in range(100)]}
                    }
                }
            },
        }
    ...
    def test_duplicate_id_handling(self, adapter):
        ...
        if doc_id2 == custom_id:
            ...
            retrieved = adapter.find_document_by_id(
                "dup_test", custom_id, "test_schema"
            )
            assert retrieved["name"] == "Second Doc"
        else:
            # If different ID returned, both documents should exist
            assert doc_id2 != custom_id
            doc1_retrieved = adapter.find_document_by_id(
                "dup_test", custom_id, "test_schema"
            )
            doc2_retrieved = adapter.find_document_by_id(
                "dup_test", doc_id2, "test_schema"
            )
