...
class TestGraphStoreAdapter:
    ...
    def test_create_edge_nonexistent_vertices(self, adapter):
        ...
        with pytest.raises(ValueError, match="From vertex .* does not exist"):
            adapter.create_edge(
                from_vertex="nonexistent1",
                to_vertex="nonexistent2",
                label="KNOWS"
            )
    ...
    def test_find_vertices(self, adapter):
        ...
        young_persons = adapter.find_vertices(
            label="Person",
            properties={"age": 25}
        )
    ...
    def test_find_edges(self, adapter):
        ...
        e1_id = adapter.create_edge(
            from_vertex=v1_id, to_vertex=v2_id, 
            label="KNOWS", properties={"since": "2020"}
        )
        e2_id = adapter.create_edge(
            from_vertex=v2_id, to_vertex=v3_id, 
            label="KNOWS", properties={"since": "2021"}
        )
        e3_id = adapter.create_edge(
            from_vertex=v1_id, to_vertex=v3_id, 
            label="WORKS_WITH", properties={"project": "AI"}
        )
    ...
    def test_traverse_with_filters(self, adapter):
        ...
        va_id = adapter.create_vertex(label="Person", properties={"name": "Alice", "age": 30})
        vb_id = adapter.create_vertex(label="Person", properties={"name": "Bob", "age": 25})
        vc_id = adapter.create_vertex(label="Person", properties={"name": "Charlie", "age": 35})
        ...
        results_strong = adapter.traverse_bfs(
            va_id,
...
class TestGraphStoreQueries:
    ...
    def _setup_sample_graph(self, adapter):
        ...
        adapter.create_edge(
            ...
            "KNOWS", properties={"since": "2020", "strength": 0.8}
        )
        adapter.create_edge(
            self.bob_id, self.charlie_id, 
            "KNOWS", properties={"since": "2021", "strength": 0.6}
        )
        adapter.create_edge(
            self.alice_id, self.techcorp_id, 
            "WORKS_FOR", properties={"role": "Engineer", "start_date": "2019-01-01"}
        )
        adapter.create_edge(
            self.charlie_id, self.techcorp_id, 
            "WORKS_FOR", properties={"role": "Manager", "start_date": "2018-01-01"}
        )
...
class TestGraphStorePerformance:
    ...
    def test_bulk_vertex_creation(self, adapter):
        ...
        for i in range(num_vertices):
            vertex_id = adapter.create_vertex(
                label="Node",
                properties={"id": i, "name": f"Node{i}", "value": i * 2}
            )
        ...
        def create_tree(adapter, depth, branching_factor, parent_id=None, level=0):
            ...
            if parent_id is None:
                ...
                root_id = adapter.create_vertex(
                    label="Node",
                    properties={"level": level, "name": f"root"}
                )
                vertex_ids.append(root_id)
                
                # Create children
                for child in create_tree(adapter, depth, branching_factor, root_id, level + 1):
                ...
                for i in range(branching_factor):
                    ...
                    for grandchild in create_tree(adapter, depth, branching_factor, child_id, level + 1):
    ...
    def test_concurrent_operations(self, adapter):
        ...
        print(f"Concurrent vertex creation: {len(all_vertex_ids)} vertices in {vertex_creation_time:.2f}s")
        ...
        if len(all_vertex_ids) >= 2:
            ...
            print(f"Concurrent edge creation: {len(all_edge_ids)} edges in {edge_creation_time:.2f}s")
        
        # Verify that most operations succeeded
        expected_vertices = num_workers * vertices_per_worker
        assert len(all_vertex_ids) >= expected_vertices * 0.8  # Allow for some concurrency issues
