...
class EndToEndTestSuite:
        ...
        try:
            ...
            for i, source_pod in enumerate(pod_names):
                ...
                for j, target_ip in enumerate(pod_ips):
                    if i != j:
                        connectivity_matrix[source_pod][target_ip] = await self._test_connectivity(target_ip, 9999)
            
            # Check cluster membership via orchestrator
            orchestrator_service = f"hmssql-orchestrator.{self.namespace}.svc.cluster.local" if self.v1 else "localhost:3000"
    ...
    async def _test_database_operations(self):
        ...
        try:
            ...
            primary_service = f"hmssql-primary.{self.namespace}.svc.cluster.local" if self.v1 else "localhost:9999"
            replica_service = f"hmssql-replica.{self.namespace}.svc.cluster.local" if self.v1 else "localhost:10000"
    ...
    async def _test_performance_benchmarks(self):
        ...
        try:
            ...
            execution_time = time.time() - start_time
            self.test_results["tests"][test_name] = {
                "status": "PASSED" if results.get("production_ready", False) else "FAILED",
                "execution_time": execution_time,
                "benchmark_results": results
            }
            
            self.logger.info(f"✅ Performance benchmarks completed ({execution_time:.2f}s)")
    ...
    async def _test_connectivity(self, host: str, port: int, timeout: float = 5.0) -> bool:
    ...
    async def _test_read_consistency(self, primary_service: str, replica_service: str):
        ...
        self.logger.info(f"Testing read consistency between {primary_service} and {replica_service}")
