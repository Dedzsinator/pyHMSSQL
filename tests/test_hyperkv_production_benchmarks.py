...
class TestHyperKVProductionBenchmarks:
    ...
    async def test_sustained_throughput_performance(self, production_server):
        ...
        assert set_throughput > 1000, f"SET throughput too low: {set_throughput:.0f} ops/sec"
        assert get_throughput > 2000, f"GET throughput too low: {get_throughput:.0f} ops/sec"
        
        print(f"Production throughput - SET: {set_throughput:.0f} ops/sec, GET: {get_throughput:.0f} ops/sec")
    ...
    async def test_concurrent_client_simulation(self, production_server):
        ...
        assert overall_throughput > 500, f"Concurrent throughput too low: {overall_throughput:.0f} ops/sec"
        assert overall_avg_latency < 30.0, f"Concurrent avg latency too high: {overall_avg_latency:.3f}ms"
        assert overall_p95 < 100.0, f"Concurrent P95 latency too high: {overall_p95:.3f}ms"
        
        print(f"Concurrent simulation ({num_clients} clients): {overall_throughput:.0f} ops/sec, avg: {overall_avg_latency:.3f}ms")
    ...
    async def test_crdt_distributed_performance(self, production_server):
        ...
        assert crdt_throughput > 1000, f"CRDT throughput too low: {crdt_throughput:.0f} ops/sec"
        assert total_counter_value == num_nodes * ops_per_node, f"Counter consistency error: {total_counter_value}"
        
        print(f"CRDT distributed performance: {crdt_throughput:.0f} ops/sec, LWW: {lww_avg:.3f}ms, Counter: {counter_avg:.3f}ms")
    ...
    async def test_memory_efficiency_under_load(self, production_server):
        ...
        memory_per_key = memory_increase / (num_sessions * keys_per_session) * 1024  # KB per key
        recovery_rate = (memory_recovered / memory_increase) * 100 if memory_increase > 0 else 0
        
        assert memory_per_key < 10.0, f"Memory per key too high: {memory_per_key:.2f} KB"
        ...
        print(f"Production memory efficiency: {memory_per_key:.2f} KB/key, {recovery_rate:.1f}% recovery")
    ...
    async def test_ttl_performance_at_scale(self, production_server):
        ...
        assert set_throughput > 500, f"TTL set throughput too low: {set_throughput:.0f} ops/sec"
        assert check_throughput > 1000, f"TTL check throughput too low: {check_throughput:.0f} ops/sec"
        assert existing_count == num_keys, f"Not all TTL keys were set: {existing_count}/{num_keys}"
        assert expired_count > num_keys * 0.95, f"TTL expiration failed: {expired_count}/{num_keys} expired"
        
        print(f"TTL at scale: set {set_throughput:.0f} ops/sec, check {check_throughput:.0f} ops/sec, {expired_count}/{num_keys} expired")
