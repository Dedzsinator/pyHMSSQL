...
from kvstore.shards import (
    AdvancedShardManager, ShardConfig, PlacementStrategy,
    AdvancedShard, ShardingError, ConsistentHashRing
)
    ...
    async def test_execute_on_shard(self, shard_config_dict):
        ...
        try:
            ...
            result = await manager.execute_on_shard(
                test_key,
                lambda shard, k=test_key, v=test_value: shard.set(k, v)
            )
            ...
            result = await manager.execute_on_shard(
                test_key,
                lambda shard, k=test_key: shard.get(k)
            )
        ...
        try:
            ...
            async def set_operation(key_suffix):
                ...
                return await manager.execute_on_shard(
                    key,
                    lambda shard, k=key, v=value: shard.set(k, v)
                )
    ...
    def test_shard_rebalancing(self, shard_config_dict):
        ...
        try:
            ...
            for i in range(50):
                ...
                asyncio.run(manager.execute_on_shard(
                    key,
                    lambda shard, k=key, v=f"value_{i}": shard.set(k, v)
                ))
