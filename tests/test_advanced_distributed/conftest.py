...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "endtoend: mark test as an end-to-end test"
    )
    config.addinivalue_line(
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if "test_" in item.name and any(marker in item.name.lower() for marker in 
                                       ["performance", "benchmark", "load", "stress", "large"]):
...
class PerformanceMeasurement:
    ...
    def assert_performance(self, min_ops_per_sec: float = 100, 
...
class AsyncTestUtils:
    ...
    @staticmethod
    async def wait_for_condition(condition_func, timeout: float = 10.0, 
...
class MockFailureInjector:
    ...
    def maybe_raise_exception(self, exception_type: type = Exception,
...
if __name__ == "__main__":
    ...
    fixtures = [
        'temp_directory', 'test_config', 'benchmark_runner',
        'test_data_generator', 'mock_network_delay', 'failure_injector',
        'async_utils', 'test_reporter', 'integration_helper'
    ]
