...
class TestWriteAheadLog:
    ...
    async def test_wal_write_entry(self, wal_config):
        ...
        try:
            ...
            seq1 = await wal.write_entry(
                WALEntryType.SET, key="user:1", value={"name": "Alice", "age": 30}
            )
            assert seq1 == 1

            # Write a DELETE entry
            seq2 = await wal.write_entry(WALEntryType.DELETE, key="user:2")
            assert seq2 == 2
    ...
    async def test_wal_checkpoint_creation(self, wal_config):
        ...
        try:
            for i in range(5):
                await wal.write_entry(
                    WALEntryType.SET, key=f"key_{i}", value=f"value_{i}"
                )
