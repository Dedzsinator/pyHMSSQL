...
class TestRunner:
    ...
    def __init__(self, test_root: str = None):
        ...
        self.components = {
            ...
            "compression": [
                "test_compression.py",
                "test_integration.py::TestShardingCompressionIntegration",
            ],
            "zerocopy": [
                "test_zerocopy.py",
                "test_integration.py::TestShardingCompressionIntegration",
            ...
            "crdt": [
                "test_crdt.py",
                "test_integration.py::TestCRDTNetworkingIntegration",
            ],
            "networking": [
                "test_networking.py",
                "test_integration.py::TestCRDTNetworkingIntegration",
            ...
            "pubsub": [
                "test_networking.py",
                "test_integration.py::TestPubSubRaftIntegration",
            ],
        }
    ...
    def run_tests(
        ...
        # Add reporting
        cmd.extend(["--junit-xml=" + str(self.report_dir / "junit.xml"), "--tb=short"])
        ...
        try:
            ...
            duration = time.time() - start_time
            return {"status": "timeout", "duration": duration, "command": cmd}
    ...
    def _get_test_files(
        self, category: Optional[str], component: Optional[str], pattern: Optional[str]
    ) -> List[str]:
    ...
    def _generate_summary_report(self, test_result: Dict[str, Any]):
        ...
        summary = {
            ...
            "success_rate": (
                passed_count / max(1, passed_count + failed_count)
    ...
    def run_benchmark_suite(self) -> Dict[str, Any]:
        ...
        for component in components_to_benchmark:
            ...
            result = self.run_tests(component=component, benchmark=True, verbose=True)
            benchmark_results[component] = result

        # Integration benchmarks
        print(f"\n--- Running Integration Benchmarks ---")
        result = self.run_tests(category="integration", benchmark=True, verbose=True)
        benchmark_results["integration"] = result

        # System benchmarks
        print(f"\n--- Running System Benchmarks ---")
        result = self.run_tests(category="endtoend", benchmark=True, verbose=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Advanced distributed database test runner"
    )
    ...
    test_group.add_argument(
        "--endtoend", action="store_true", help="Run end-to-end tests"
    )
    test_group.add_argument(
    ...
    parser.add_argument(
        "--benchmark-suite", action="store_true", help="Run complete benchmark suite"
    )
