...
from kvstore.raft import (
    RaftNode, RaftConfig, RaftState, LogEntry,
...
class TestRaftAppendEntries:
    ...
    def test_append_entries_request(self):
        ...
        entries = [
            LogEntry(term=1, index=0, command={"type": "SET", "key": "a", "value": "1"}),
            LogEntry(term=1, index=1, command={"type": "SET", "key": "b", "value": "2"})
        ]
...
class TestRaftLogReplication:
    ...
    def test_append_entries_log_consistency_check(self, leader_follower_setup):
        ...
        follower.log.append(LogEntry(
            term=1, 
            index=0, 
            command={"type": "SET", "key": "old", "value": "data"}
        ))
        ...
        request = AppendEntriesRequest(
            ...
            entries=[LogEntry(term=1, index=1, command={"type": "SET", "key": "new", "value": "data"})],
            leader_commit=0
        )
...
class TestRaftErrorHandling:
    ...
    def test_concurrent_election_handling(self, raft_node):
        ...
        request1 = VoteRequest(term=1, candidate_id="candidate1", last_log_index=0, last_log_term=0)
        request2 = VoteRequest(term=1, candidate_id="candidate2", last_log_index=0, last_log_term=0)
    ...
    def test_memory_usage_with_large_log(self, performance_raft_node):
        ...
        for i in range(entry_count):
            entry = LogEntry(
                term=1,
                index=i,
                command={"type": "SET", "key": f"large_key_{i}", "value": f"large_value_{i}"}
            )
            node.log.append(entry)
