...
try:
    from test_config import TEST_SIZES, BPTREE_ORDERS, TEST_DIMENSIONS, PERFORMANCE_TIMEOUT, log_test_config
    log_test_config()
except ImportError:
    # Fallback configuration if test_config is not available
    TEST_SIZES = {'small': 100, 'medium': 500, 'large': 1000, 'grid_size': 20, 'stress_size': 1000}
...
class TestBPTreeMultidimensional:
    ...
    def test_large_scale_multidimensional(self):
        ...
        assert success_rate >= 0.95, f"Success rate {success_rate:.2%} too low (failed: {failed_searches}/{len(points)})"
...
class TestBPTreeCompositeOperations:
    ...
    def test_composite_insertion(self):
        ...
        if hasattr(tree, 'insert_composite'):
            ...
            tree.insert_composite([5.0, 6.0], "multi_2")  # Same dimension as first multi insert
...
class TestBPTreeErrorHandling:
    ...
    def test_invalid_inputs(self):
        ...
        with pytest.raises(ValueError):  # String keys converted to float, this raises ValueError
...
class TestBPTreeIntegration:
    ...
    def test_memory_efficiency(self):
        ...
        sample_indices = [0, test_size // 4, test_size // 2, 3 * test_size // 4, test_size - 1]
