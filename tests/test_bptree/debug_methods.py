...
try:
    ...
    if search_result == "value1":
        ...
        print(
            f"   ✗ Basic functionality failed: expected 'value1', got {search_result}"
        )
except Exception as e:
...
if hasattr(tree, "insert_multidim"):
    try:
        ...
        if search_result == "point1":
            ...
            print(
                f"   ✗ Multidimensional functionality failed: expected 'point1', got {search_result}"
            )
    except Exception as e:
...
if hasattr(tree, "range_query_multidim"):
    try:
        ...
        if hasattr(tree, "insert_multidim"):
            ...
            print(
                f"   Multidim range query results: {len(results) if results else 0} items"
            )
