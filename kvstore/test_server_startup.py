...
async def test_server_startup():
    ...
    try:
        ...
        assert all(k.startswith("scan") for k in keys), f"All keys should start with 'scan': {keys}"
        ...
        assert 'total_operations' in stats['stats'], "Stats should include total_operations"
        assert stats['stats']['total_operations'] > 0, "Should have some operations recorded"
        logger.info(f"✓ Statistics: {stats['stats']['total_operations']} operations, {stats['stats'].get('memory_usage', 0)} bytes memory")
