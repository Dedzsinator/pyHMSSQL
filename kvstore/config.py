...
class HyperKVConfig:
    ...
    def from_env(cls) -> "HyperKVConfig":
        ...
        config.storage.backend = os.getenv(
            "HYPERKV_STORAGE_BACKEND", config.storage.backend
        )

        # Cluster settings
        config.cluster.enabled = (
            os.getenv("HYPERKV_CLUSTER_ENABLED", "false").lower() == "true"
        )
        ...
        # Kubernetes settings
        config.cluster.k8s_namespace = os.getenv(
            "HYPERKV_K8S_NAMESPACE", config.cluster.k8s_namespace
        )
        config.cluster.k8s_service_name = os.getenv(
            "HYPERKV_K8S_SERVICE_NAME", config.cluster.k8s_service_name
        )
