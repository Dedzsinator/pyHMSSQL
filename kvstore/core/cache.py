...
class LFUStrategy(EvictionStrategy):
    ...
    def select_victims(self, count: int, volatile_only: bool = False) -> List[str]:
        with self._lock:
            ...
            while (
                len(victims) < count
                and freq < max(self._freq_to_keys.keys(), default=0) + 1
            ):
    ...
    def on_access(self, key: str, entry: CacheEntry):
        with self._lock:
            if key in self.t1:
                ...
                self.p = min(
                    self.cache_size, self.p + max(1, len(self.b2) // len(self.b1))
                )
                del self.b1[key]
                self.t2[key] = entry.last_accessed
            elif key in self.b2:
...
class CacheManager:
    ...
    def _evict_entries(self):
        ...
        # Select victims
        victims = self._strategy.select_victims(self.eviction_batch_size, volatile_only)
    ...
    def clear(self):
        ...
        with self._lock:
            ...
            # Reset statistics
            self.stats.update(
                {"total_entries": 0, "memory_usage": 0, "hits": 0, "misses": 0}
            )
