...
class GorillaCodec(CompressionCodec):
    ...
    def decompress(
        ...
        stored_metadata = json.loads(
            compressed_data[4 : 4 + metadata_len].decode("utf-8")
        )
        ...
        compressed_values = compressed_data[
            offset + timestamp_size : offset + timestamp_size + values_size
        ]

        # Decompress
        timestamps = self._decompress_timestamps(
            compressed_timestamps, stored_metadata["count"]
        )
    ...
    def decompress(
        ...
        if data_type == "int64":
            ...
            if count > 1:
                ...
                delta_size = struct.unpack(">B", compressed_data[offset : offset + 1])[
                    0
                ]
                offset += 1
                ...
                for i in range(1, count):
                    if delta_size == 1:
                        delta = struct.unpack(
                            ">b", compressed_data[offset : offset + 1]
                        )[0]
                        offset += 1
                    elif delta_size == 2:
                        delta = struct.unpack(
                            ">h", compressed_data[offset : offset + 2]
                        )[0]
                        offset += 2
                    elif delta_size == 4:
                        delta = struct.unpack(
...
class DictionaryCodec(CompressionCodec):
    ...
    def compress(self, data: Union[bytes, List[Any]]) -> CompressionResult:
        ...
        result_data = pickle.dumps({"dictionary": unique_values, "encoded": encoded})
...
def compress_time_series(
    timestamps: List[int], values: List[float]
) -> CompressionResult:
    """Compress time series data using Gorilla algorithm"""
    codec = GorillaCodec()
...
def compress_numeric_sequence(
    values: List[Union[int, float]], use_integers: bool = True
) -> CompressionResult:
