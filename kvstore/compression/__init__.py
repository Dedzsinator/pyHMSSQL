...
class CompressionManager:
    ...
    def _detect_available_algorithms(self) -> Dict[CompressionType, bool]:
        ...
        logger.info(f"Available compression algorithms: {[k.value for k, v in algorithms.items() if v]}")
        return algorithms
    ...
    def _serialize_data(self, data: Any) -> Tuple[bytes, Dict[str, Any]]:
        ...
        if isinstance(data, bytes):
            ...
            serialized_data = struct.pack(f"{'d' if isinstance(data, float) else 'q'}", data)
    ...
    def _select_adaptive_algorithm(self, data_size: int, data_type: str, metadata: Dict[str, Any] = None) -> CompressionType:
    ...
    def _decompress_with_algorithm(self, data: bytes, algorithm: CompressionType) -> bytes:
    ...
    def compress(self, data: Any, algorithm: Optional[CompressionType] = None) -> CompressionResult:
        """
        ...
        try:
            with self._lock:
                ...
                if not self._available_algorithms.get(algorithm, False):
                    ...
                    fallback_algorithm = self._select_adaptive_algorithm(original_size, 
                                                                        serialization_metadata.get("type", "unknown"))
                    logger.warning(f"Algorithm {algorithm.value} not available, using {fallback_algorithm.value}")
                    algorithm = fallback_algorithm
                    
                # Compress data
                compressed_data = self._compress_with_algorithm(serialized_data, algorithm)
    ...
    def decompress(self, compressed_data: bytes, algorithm: CompressionType, metadata: Dict[str, Any]) -> Any:
        """
        ...
        try:
            with self._lock:
                ...
                # Decompress data
                decompressed_data = self._decompress_with_algorithm(compressed_data, actual_algorithm)
    ...
    def compress_stream(self, data_iterator: Iterator[Any], algorithm: Optional[CompressionType] = None) -> Iterator[CompressionResult]:
        """
    ...
    def decompress_stream(self, compressed_iterator: Iterator[Tuple[bytes, CompressionType, Dict]]) -> Iterator[Any]:
        """
    ...
    def benchmark_algorithms(self, test_data: Any, algorithms: Optional[List[CompressionType]] = None) -> Dict[CompressionType, Dict[str, float]]:
        """
        ...
        if algorithms is None:
            algorithms = [algo for algo, available in self._available_algorithms.items() if available]
        ...
        for algorithm in algorithms:
            try:
                ...
                if algorithm in [CompressionType.GORILLA, CompressionType.DELTA, CompressionType.RLE]:
                    continue
                ...
                # Decompress
                start_time = time.time()
                decompressed = self.decompress(result.compressed_data, algorithm, result.metadata)
                decompress_time = time.time() - start_time
                
                # Verify correctness
                if decompressed != test_data:
                    logger.warning(f"Algorithm {algorithm.value} failed correctness test")
                    continue
                ...
                results[algorithm] = {
                    ...
                    "compression_speed_mbps": (result.original_size / compress_time) / (1024 * 1024),
                    "decompression_speed_mbps": (result.original_size / decompress_time) / (1024 * 1024),
                }
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if stats["total_original_bytes"] > 0:
                stats["overall_compression_ratio"] = stats["total_compressed_bytes"] / stats["total_original_bytes"]
            else:
                stats["overall_compression_ratio"] = 0.0
                
            if stats["compression_times"]:
                stats["avg_compression_time"] = sum(stats["compression_times"]) / len(stats["compression_times"])
            ...
            if stats["decompression_times"]:
                stats["avg_decompression_time"] = sum(stats["decompression_times"]) / len(stats["decompression_times"])
            ...
            stats["available_algorithms"] = [algo.value for algo, available in self._available_algorithms.items() if available]
...
class GorillaCodec:
    ...
    def compress(self, time_series_data: Union[List[float], List[Tuple]]) -> CompressionResult:
        """
        ...
        if isinstance(time_series_data[0], (tuple, list)) and len(time_series_data[0]) == 2:
        ...
        compressed_timestamps = self._compress_timestamps(timestamps) if has_timestamps else b''
        ...
        combined_data = struct.pack('<I', len(compressed_timestamps)) + compressed_timestamps + compressed_values
    ...
    def decompress(self, compressed_data: bytes, metadata: Dict[str, Any]) -> Union[List[float], List[Tuple]]:
        ...
        # Decompress
        timestamps = self._decompress_timestamps(compressed_timestamps) if has_timestamps else None
...
class DeltaCodec:
    ...
    def _compress_numeric_sequence(self, data: List[Union[int, float]]) -> CompressionResult:
    ...
    def _decompress_numeric_delta(self, data: Dict[str, Any]) -> List[Union[int, float]]:
    ...
    def _decompress_mixed_delta(self, data: Dict[str, Any]) -> List[Any]:
        ...
        for item_type, value in items:
            if item_type == "full":
                ...
                if result and isinstance(result[-1], (int, float)) and isinstance(value, (int, float)):
...
def decompress_data(compressed_data: bytes, algorithm: CompressionType, metadata: Dict[str, Any]) -> Any:
    """Decompress data using a default compression manager"""
    manager = create_compression_manager()
    return manager.decompress(compressed_data, algorithm, metadata)


def benchmark_compression(test_data: Any, algorithms: Optional[List[CompressionType]] = None) -> Dict[CompressionType, Dict[str, float]]:
    """Benchmark compression algorithms on test data"""
    manager = create_compression_manager()
    return manager.benchmark_algorithms(test_data, algorithms)


def detect_best_algorithm(test_data: Any, strategy: CompressionStrategy = CompressionStrategy.BALANCED) -> CompressionType:
    """
