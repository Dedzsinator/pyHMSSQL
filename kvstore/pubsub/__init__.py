...
class PubSubManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.max_subscribers_per_channel = self.config.get(
        ...
        with self._lock:
            if channel_name not in self.channels:
                ...
                self.channels[channel_name] = Channel(
                    channel_name, self.message_buffer_size
                )
                self.stats["total_channels"] += 1
        ...
        try:
            with self._lock:
                ...
                if (
                    ...
                    >= self.max_subscribers_per_channel * self.max_channels
                ):
                    return None
                ...
                if (
                    channel_obj.get_subscriber_count()
                ...
                logger.debug(
                    f"Subscriber {subscriber_id} subscribed to channel {channel}"
                )
    ...
    async def psubscribe(
        ...
        try:
            with self._lock:
                ...
                logger.debug(
                    f"Subscriber {subscriber_id} subscribed to pattern {pattern}"
                )
                return subscriber_id
    ...
    async def unsubscribe(self, channel: str, subscription_id: str) -> bool:
        ...
        try:
            with self._lock:
                ...
                empty_patterns = [
                    p for p, subs in self.pattern_subscribers.items() if len(subs) == 0
                ]
                ...
                logger.debug(
                    f"Subscriber {subscription_id} unsubscribed from {channel}"
                )
                return True
    ...
    async def publish(self, channel: str, data: Any) -> int:
        ...
        try:
            ...
            logger.debug(
                f"Published message to {channel}, notified {subscribers_notified} subscribers"
            )
