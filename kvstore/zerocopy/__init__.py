...
class BufferPool:
    ...
    def __init__(self, min_buffers: int = 2, max_buffers: int = 100, default_size: int = 1024 * 1024):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            buffer_sizes = [self.buffer_info[bid].size for bid in self.buffer_info.keys()]
            # Active buffers = total allocated buffers when none are in use, otherwise in-use count
            in_use_count = sum(1 for info in self.buffer_info.values() if info.ref_count > 0)
...
class MemoryMappedBuffer:
    ...
    def open(self):
        ...
        try:
            ...
            map_size = self.requested_size if self.requested_size and self.requested_size <= file_size else file_size
            self._size = map_size
            
            self.mmap_obj = mmap.mmap(self.file_handle.fileno(), map_size, access=access_mode)
...
class ZeroCopyManager:
    ...
    def map_file(self, file_path: str, mode: str = "r", size: int = None) -> MemoryMappedBuffer:
        """Memory map a file"""
        with self.lock:
