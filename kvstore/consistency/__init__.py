...
class HintedHandoff:
    ...
    def store_hint(
        self, failed_node: str, hint_data: Dict[str, Any], target_node: str
    ) -> bool:
    ...
    def cleanup_expired_hints(self):
        ...
        with self.lock:
            ...
            for node_id in list(self.hints.keys()):
                self.hints[node_id] = [
                    hint
                    for hint in self.hints[node_id]
                    if current_time - hint["timestamp"] < self.hint_ttl_ms
                ]
...
class ReadRepair:
    ...
    async def perform_repair(
        self, key: str, responses: List[ReplicaResponse], replica_manager
    ) -> bool:
        ...
        try:
            ...
            for response in responses:
                if response.success and response.timestamp < latest_response.timestamp:
            ...
            for node_id in repair_nodes:
                task = asyncio.create_task(
                    replica_manager.repair_replica(
                        node_id, key, latest_response.value, latest_response.timestamp
                    )
                )
            ...
            try:
                await asyncio.wait_for(
                    asyncio.gather(*repair_tasks, return_exceptions=True), timeout=2.0
                )
                ...
                logger.debug(
                    f"Read repair performed for key {key} on {len(repair_nodes)} nodes"
                )
                return True
...
class ConsistencyManager:
    ...
    def calculate_required_responses(
        self,
        consistency_level: ConsistencyLevel,
        total_replicas: int,
        datacenter_replicas: Dict[str, int] = None,
    ) -> int:
        ...
        if consistency_level == ConsistencyLevel.ONE:
            ...
            local_replicas = (
                datacenter_replicas.get("local", total_replicas)
                if datacenter_replicas
                else total_replicas
            )
    ...
    async def coordinate_read(
        ...
        try:
            ...
            for completed_task in asyncio.as_completed(
                tasks, timeout=self.config.timeout_ms / 1000
            ):
                try:
        ...
        if success:
            ...
            if successful_responses_list:
                latest_response = max(
                    successful_responses_list, key=lambda r: r.timestamp
                )
                ...
                if (
                    ...
                    and len(successful_responses_list) > 1
                    and replica_manager
                ):
    ...
    async def _read_from_replica(
        self, replica_id: str, key: str, replica_manager
    ) -> ReplicaResponse:
        ...
        try:
            ...
            latency = (time.perf_counter() - start_time) * 1000
            return ReplicaResponse(
                node_id=replica_id, success=False, error=str(e), latency_ms=latency
            )

    async def _write_to_replica(
        self, replica_id: str, key: str, value: Any, timestamp: int, replica_manager
    ) -> ReplicaResponse:
        ...
        stats.update(
            {
                ...
                "hinted_handoff_size": sum(
                    len(hints) for hints in self.hinted_handoff.hints.values()
                ),
                "config": {
...
def create_strong_consistency() -> ConsistencyManager:
    ...
    config = ConsistencyConfig(
        level=ConsistencyLevel.ALL, read_repair=True, hinted_handoff=True
    )
...
def create_eventual_consistency() -> ConsistencyManager:
    ...
    config = ConsistencyConfig(
        level=ConsistencyLevel.ANY, read_repair=True, hinted_handoff=True
    )
    return ConsistencyManager(config)
...
def create_balanced_consistency() -> ConsistencyManager:
    ...
    config = ConsistencyConfig(
        level=ConsistencyLevel.QUORUM, read_repair=True, hinted_handoff=True
    )
    return ConsistencyManager(config)
