...
class RocksDBBackend(StorageBackend):
    ...
    def __init__(self, db_path: str, options: Dict[str, Any] = None):
        ...
        opts.target_file_size_base = self.options.get('target_file_size_base', 64 * 1024 * 1024)
...
class SnapshotManager:
    ...
    async def create_snapshot(self, data: Dict[str, Any], metadata: Dict[str, Any] = None) -> str:
        """Create a snapshot"""
...
class StorageEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if config.get('aof_enabled', True):
            ...
            self.aof_writer = AOFWriter(
                str(aof_path),
                config.get('aof_fsync_policy', 'everysec')
            )
        ...
        if config.get('snapshot_enabled', True):
            ...
            self.snapshot_manager = SnapshotManager(
                str(snapshot_dir),
                config.get('snapshot_compression', True)
            )
    ...
    async def initialize(self):
        ...
        if self.snapshot_manager:
            ...
            if latest_snapshot:
                snapshot_data = await self.snapshot_manager.load_snapshot(latest_snapshot)
                if snapshot_data:
    ...
    async def set(self, key: str, value: bytes) -> None:
        ...
        # Log to AOF
        if self.aof_writer:
            command = {'type': 'SET', 'key': key, 'value': value.decode('utf-8', errors='ignore')}
