...
class WALEntry:
    ...
    def from_bytes(cls, data: bytes) -> 'WALEntry':
        ...
        if calc_checksum != checksum:
            raise ValueError(f"WAL entry checksum mismatch: {calc_checksum} != {checksum}")
...
class WALSegment:
    ...
    async def read_entries(self, from_sequence: Optional[int] = None) -> AsyncIterator[WALEntry]:
...
class WriteAheadLog:
    ...
    async def write_entry(self, entry_type: WALEntryType, key: Optional[str] = None,
                         value: Optional[Any] = None, transaction_id: Optional[str] = None,
        ...
        try:
            ...
            if success:
                ...
                if (self.active_segment.entry_count % 
                    self.config.rotation_check_interval == 0):
    ...
    async def read_entries(self, from_sequence: Optional[int] = None,
                          to_sequence: Optional[int] = None) -> AsyncIterator[WALEntry]:
    ...
    async def checkpoint(self, sequence_number: int):
        ...
        await self.write_entry(
            ...
            metadata={'checkpoint_sequence': sequence_number}
        )
    ...
    async def truncate_before(self, sequence_number: int):
        ...
        for segment_id, segment in self.segments.items():
            if (segment.last_sequence is not None and 
                segment.last_sequence < sequence_number and
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            'active_segment_id': self.active_segment.segment_id if self.active_segment else None,
            'active_segment_size': self.active_segment.current_size if self.active_segment else 0,
            'config': {
