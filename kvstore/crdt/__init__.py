...
class LWWElementSet(CRDTValue):
    ...
    def value(self) -> set:
        ...
        with self._lock:
            return {element.value for element in self._elements.values() if not element.deleted}
...
class ORSet(CRDTValue):
    ...
    def __init__(self, node_id: str):
        self.node_id = node_id
        self.added_elements: List[Tuple[Any, str]] = []  # List of (element, unique_tag) pairs
        self.removed_tags: Set[str] = set()  # Set of removed tags
        self._element_tags: Dict[Any, Set[str]] = defaultdict(set)  # element -> set of tags for quick lookup
        
        # Compatibility attributes for to_dict/from_dict methods
        self.added: Dict[str, Set[str]] = defaultdict(set)  # element -> set of added tags
        self.removed: Dict[str, Set[str]] = defaultdict(set)  # element -> set of removed tags
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                'type': 'or_set',
                'node_id': self.node_id,
                'added_elements': [(str(elem), tag) for elem, tag in self.added_elements],
            ...
            for item in other_added:
                ...
                try:
                    if isinstance(element, str) and element.isdigit():
                        element = int(element)
                    elif isinstance(element, str) and element.replace('.', '').isdigit():
                ...
                if name in self.crdts:
                    ...
                    if hasattr(existing_crdt, 'merge'):
                        ...
                        if crdt_type == 'lww_element_set':
                            temp_crdt = LWWElementSet.from_dict(crdt_state, existing_crdt.clock, existing_crdt.node_id)
                    ...
                    if crdt_type == 'lww_element_set':
                        ...
                        self.crdts[name] = LWWElementSet.from_dict(crdt_state, hlc, self.node_id)
                    elif crdt_type == 'or_set':
