...
class TcpServer:
    ...
    async def _handle_client(
        ...
        client = ClientConnection(
            client_id=client_id, remote_addr=remote_addr, connected_at=time.time()
        )
            ...
            async for data in reader:
                ...
                for command in commands:
                    if isinstance(command, list) and command:
                        try:
                            response = await self.handler.handle_command(
                                client_id, command
                            )
                            ...
                            logger.error(
                                f"Error handling command from {client_id}: {e}"
                            )
                        ...
                        if self.command_handler:
                            try:
                                response = await self.command_handler(
                                    command[0], command[1:], client_id
                                )
                                response_bytes = RESPProtocol.encode_response(response)
                                writer.write(response_bytes)
                                await writer.drain()
                            except Exception as e:
                                error_response = RESPProtocol.encode_error(
                                    f"ERR {str(e)}"
                                )
