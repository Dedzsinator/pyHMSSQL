...
class ShardMigrationManager:
    ...
    def start(self):
        ...
        self.running = True
        self.worker_thread = threading.Thread(target=self._migration_worker, daemon=True)
...
class ShardLoadBalancer:
    ...
    def __init__(self, shard_manager):
        self.shard_manager = shard_manager
        self.load_stats = defaultdict(lambda: {'cpu': 0.0, 'memory': 0.0, 'operations': 0})
    ...
    def _check_rebalancing(self):
        ...
        max_load = max((stats['cpu'] for stats in self.load_stats.values()), default=0.0)
        min_load = min((stats['cpu'] for stats in self.load_stats.values()), default=0.0)
        
        if max_load > self.rebalance_threshold and (max_load - min_load) > 0.3:
            logger.info(f"Load imbalance detected: max={max_load:.2f}, min={min_load:.2f}")
...
class AdvancedShardManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if ZEROCOPY_AVAILABLE and ZeroCopyManager and self.config.get('enable_zero_copy', True):
    ...
    def start(self):
        ...
        if CONSISTENCY_AVAILABLE and ConsistencyManager and self.config.get('enable_consistency', True):
    ...
    def get_shard_for_key(self, key: str, consistency_level: Optional[Any] = None) -> int:
    ...
    async def _execute_cross_shard_operation(
        self,
        operation: Callable,
        consistency_level: Optional[Any],
        *args,
        **kwargs
    ) -> Any:
...
__all__ = [
    ...
    'ShardMigrationManager', 'ShardLoadBalancer', 'ShardPerformanceMonitor'
]
