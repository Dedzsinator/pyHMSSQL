...
class AdvancedShardManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.placement_strategy = self.config.get(
            "placement_strategy", PlacementStrategy.NUMA_AWARE
        )
        ...
        self.migration_manager = (
            ShardMigrationManager(self) if ShardMigrationManager else None
        )
        ...
        self.performance_monitor = (
            ShardPerformanceMonitor(self) if ShardPerformanceMonitor else None
        )
    ...
    def start(self):
        ...
        for shard_id in range(self.num_shards):
            try:
                ...
                memory_per_shard = self.config.get(
                    "memory_per_shard", 256 * 1024 * 1024
                )
    ...
    async def _execute_single_shard_operation(
        ...
        if shard_id not in self.shards:
            logger.error(
                f"Shard {shard_id} not found in available shards: {list(self.shards.keys())}"
            )
        ...
        if self.wal:
            ...
            def stop_wal():
                try:
                    ...
                    logger.warning(
                        f"Error stopping WAL in shard {self.config.shard_id}: {e}"
                    )
                    ...
                    try:
                        ...
                        try:
                            ...
                            loop.run_until_complete(
                                self.wal.write_entry(
                                    WALEntryType.SET, key=key, value=wal_value
                                )
                            )
                        finally:
                            loop.close()
                        ...
                        logger.warning(
                            f"WAL write failed for shard {self.config.shard_id}: {e}"
                        )
            ...
            try:
                ...
                def run_wal_write():
                    try:
                        ...
                        try:
                            loop.run_until_complete(
                                self.wal.write_entry(WALEntryType.DELETE, key=key)
                            )
                        finally:
                            loop.close()
                    except Exception as e:
                        logger.warning(
                            f"WAL delete write failed for shard {self.config.shard_id}: {e}"
                        )
                ...
                if not hasattr(self, "_memory_wal"):
                    self._memory_wal = []
                self._memory_wal.append(
                    {"operation": "DELETE", "key": key, "timestamp": time.time()}
                )
