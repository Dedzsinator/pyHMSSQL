...
class RaftNode:
    ...
    def __init__(self, node_id: str, peers: List[str], config: Optional[RaftConfig] = None):
        ...
        self.send_vote_request: Optional[Callable[[str, VoteRequest], Future[VoteResponse]]] = None
        self.send_append_entries: Optional[Callable[[str, AppendEntriesRequest], Future[AppendEntriesResponse]]] = None
        ...
        try:
            with self._lock:
                ...
                if next_index <= len(self.log):
                    end_index = min(next_index + self.config.max_log_entries_per_request, len(self.log) + 1)
    ...
    async def _handle_append_entries_response(self, peer: str, request: AppendEntriesRequest, 
    ...
    def _update_commit_index(self):
        ...
        for index in range(self.commit_index + 1, len(self.log) + 1):
            ...
            if count > len(self.peers) // 2 + 1:
                if index <= len(self.log) and self.log[index - 1].term == self.current_term:
        ...
        if vote_tasks:
            ...
            for response in responses:
                if isinstance(response, VoteResponse) and response.vote_granted:
                    votes += 1
                elif isinstance(response, VoteResponse) and response.term > self.current_term:
                    self._step_down(response.term)
                    return
    ...
    def restore_persistent_state(self, state: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self.log = [LogEntry.from_dict(entry_data) for entry_data in state.get('log', [])]
