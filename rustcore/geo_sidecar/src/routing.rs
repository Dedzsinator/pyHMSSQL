...
impl RoutingEngine {
    ...
    pub fn update_replicas(&mut self, replicas: Vec<ReplicaInfo>) -> Result<()> {
        ...
        tracing::info!("Updated routing table with {} replicas", self.replicas.len());
        Ok(())
    }
    ...
    pub fn route_request(
        ...
        let healthy_replicas: Vec<_> = self.replicas
            .iter()
        ...
        let distance_km = geo_resolver.calculate_distance(
            &client_location,
            &selected_replica.geo_location
        );
